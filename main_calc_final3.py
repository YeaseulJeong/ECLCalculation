...
def load_data():
    ...
    try:
        ...
        #기본 ECL 데이터
        basic_data = pd.read_csv('ECL.csv', encoding='utf-8')
        # 시나리오 1 데이터
        scenario1_data = pd.read_csv('UpdatedECL_scn1.csv', encoding='utf-8')
        # 시나리오 2 데이터  
        scenario2_data = pd.read_csv('UpdatedECL_scn2.csv', encoding='utf-8')
        # 발생 신용손실 데이터
        actual_data = pd.read_csv('IncurredLoss.csv', encoding='utf-8')
        # 거시경제 지표 데이터
        macro_data = pd.read_csv('Macroeconomics.csv', encoding='utf-8')
...
def main():
    ...
    # Y축 설정
    fig_trends.update_yaxes(title_text="발생신용손실(단위: 백만원)", secondary_y=False)
    ...
    <div class="insight-box">
        <h3>📊 분석 결과 및 인사이트</h3>
        <ul>
            <li><strong>경제지표 반영을 통한 예측 정확도 향상 입증</strong>: ECL 모델 개선 효과 확인</li>
            <li><strong>시나리오 2(실업률+경기판단CSI) 의 우수성 입증</strong>: 실업률, 경기판단CSI를 반영한 개선된 시나리오 2 ECL이 기존 모델보다 우수한 성능을 보임</li>
            <li><strong>모델의 한계점 확인</strong>: 급작스러운 금융위기(2008년, 2012년) 시 예측 한계 노출, 경기 변동에 따른 동적 조정 부족으로 시차 효과 발생</li>
