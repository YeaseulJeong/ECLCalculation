...
def generate_sample_data():
    ...
    macro_data = pd.DataFrame({
        ...
        '경제성장률': [0.07, 0.037, 0.025, 0.033, 0.032],
        '실업률': [0.037, 0.034, 0.032, 0.031, 0.035],
        '기준금리': [2.5, 3.25, 2.75, 2.5, 2.0],
        '현재경기판단CSI': [104.0, np.nan, 90.0, np.nan, 66.0],
        '기업부도율': [0.02, 0.02, 0.03, 0.03, 0.03]
    })
...
def main():
    ...
    # Y축 설정
    fig_trends.update_yaxes(title_text="발생신용손실 (백만원)", secondary_y=False)
    ...
    # 3. 시나리오별 ECL 상관관계 예측
    st.subheader("🔮 시나리오별 ECL 상관관계 예측")
    
    # 시나리오 분석을 위한 탭 생성
    tab1, tab2, tab3 = st.tabs(["💹 실업률↑ + 금리↑", "📉 성장률↓ + 실업률↑", "📊 복합 리스크 매트릭스"])
    
    with tab1:
        st.markdown("### 실업률 상승 + 금리 상승 = 신용위험 기하급수적 증가")
        ...
        fig_scenario1 = px.imshow(
            ...
            title="실업률 vs 금리 위험도 매트릭스",
            labels={'x': '기준금리 (%)', 'y': '실업률 (%)', 'color': '위험도 승수'}
        )
        
        fig_scenario1.update_layout(height=400)
        st.plotly_chart(fig_scenario1, use_container_width=True)
    
    with tab2:
        st.markdown("### 경제성장률 하락 + 실업률 상승 = 복합 리스크 시너지")
        ...
        fig_scenario2 = px.imshow(
            ...
            title="경제성장률 vs 실업률 시너지 효과",
            labels={'x': '경제성장률 (%)', 'y': '실업률 (%)', 'color': '시너지 위험도'}
        )
        
        fig_scenario2.update_layout(height=400)
        st.plotly_chart(fig_scenario2, use_container_width=True)
    
    with tab3:
        st.markdown("### 복합 리스크 매트릭스")
        
        # 실제 데이터를 기반으로 한 위험도 계산
        risk_data = []
        for year in range(2010, 2015):
            macro_year = macro_data[macro_data['YEAR'] == year]
            if not macro_year.empty:
                growth = macro_year.iloc[0]['경제성장률'] * 100
                unemployment = macro_year.iloc[0]['실업률'] * 100
                interest = macro_year.iloc[0]['기준금리']
                
                # 복합 위험도 계산
                growth_risk = max(0, (5 - growth) / 5)  # 성장률 5% 이하일 때 위험
                unemployment_risk = max(0, (unemployment - 3) / 2)  # 실업률 3% 이상일 때 위험
                interest_risk = max(0, (interest - 2.5) / 2)  # 금리 2.5% 이상일 때 위험
                
                composite_risk = (growth_risk + unemployment_risk + interest_risk) / 3
